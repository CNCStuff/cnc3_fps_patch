...
static void KW_STDCALL kw_pace_client_frame(void *engine_pointer) {
    ...
    DWORD previous_ms = *(volatile DWORD *)kw_game_address(
    ...
    float scale = *(volatile float *)kw_game_address(g_game, pacing->network_scale);
    float logic_ms = *(volatile float *)kw_game_address(
    ...
    *(volatile DWORD *)kw_game_address(g_game, pacing->last_frame_duration_ms) = work_ms;
    *(volatile DWORD *)kw_game_address(g_game, pacing->last_wait_ms) = wait_ms;
    *(volatile DWORD *)kw_game_address(g_game, pacing->total_wait_ms) += wait_ms;
    *(volatile DWORD *)kw_game_address(g_game, pacing->previous_frame_time_ms) = after_ms;
}
...
static BOOL kw_build_stub(void) {
    ...
    kw_encode_u32(&stub[2],
                  (kw_u32)(uintptr_t)kw_game_address(g_game, pacing->enforce_limit_flag));
    stub[6] = 0x00;
    ...
    stub[15] = 0xE9;
    kw_encode_rel32(&stub[16], stub + 20, kw_game_address(g_game, pacing->history_path));
    stub[20] = 0xE9;
    kw_encode_rel32(&stub[21], stub + 25, kw_game_address(g_game, pacing->no_limit_path));
